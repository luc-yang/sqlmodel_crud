...
class Quota(SQLModel, table=True):
    ...
    volume_name: str = Field(index=True, description="定额所在册的名称")
    chapter_name: str = Field(index=True, description="定额所在章的名称")
    code: str = Field(unique=True, index=True, description="定额标准编码")
